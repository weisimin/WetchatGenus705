...
//        Log.d(TAG, "登录页面，文字：" + ((TextView) view).getText() + " 被点击了。");
    ...
    private void onClickLogin(View view) {
        ...
        if (!RegexUtils.isUsername(strName)) {
            ...
            Toast.makeText(this, "账号格式不正确！", Toast.LENGTH_LONG).show();
            Looper.loop();
        ...
        if (strPassword.length() < 6 || strPassword.length() > 20) {
            Looper.prepare();
            Toast.makeText(this, "密码长度为6到20位！", Toast.LENGTH_LONG).show();
            Looper.loop();
...
{
    Looper.prepare();
    Toast.makeText(this, "登陆失败，没有错误消息", Toast.LENGTH_LONG).show();
    Looper.loop();
    return;

}

if      (Loginr.contains("错误"))
{
    Looper.prepare();
    Toast.makeText(this, Loginr, Toast.LENGTH_LONG).show();
    Looper.loop();
    return;
